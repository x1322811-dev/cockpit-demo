...
export default function PassengerZone() {
  const { climate, windows, seatPassenger, patchClimate, patchWindows, patchSeatPassenger } = useAppStore(
    useShallow(s => ({
      climate:            s.vehicle.climate,
      windows:            s.vehicle.windows,
      seatPassenger:      s.vehicle.seatPassenger,
      patchClimate:       s.patchClimate,
      patchWindows:       s.patchWindows,
      patchSeatPassenger: s.patchSeatPassenger,
    }))
  );
  ...
  const toggleZeroGravity = () => {
    if (!seatPassenger.zeroGravity) {
      setSavedSeat({ foreAft: seatPassenger.foreAft, recline: seatPassenger.recline });
      patchSeatPassenger({ zeroGravity: true, foreAft: 100, recline: 170 });
    } else {
      patchSeatPassenger({ zeroGravity: false, ...(savedSeat ?? { foreAft: 50, recline: 110 }) });
      setSavedSeat(null);
    }
  };
    ...
    <div className="swiss-card" style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div>
        <div style={SECTION_HEADER}>
          ...
          <button
            className={`swiss-chip${climate.acRear ? ' active' : ''}`}
            style={{ cursor: 'pointer' }}
            onClick={() => patchClimate({ acRear: !climate.acRear })}
          >
            {climate.acRear ? 'ON' : 'OFF'}
          </button>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', paddingBottom: 10 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 4 }}>
            ...
            <FanBlocks level={climate.fanSpeedRear} onChange={v => patchClimate({ fanSpeedRear: v })} />
          </div>

          {/* 中：温度 */}
          <div style={{ flex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
            <span className="swiss-label">{climate.tempSync ? <span style={{ color: 'var(--accent)' }}>SYNC</span> : '温度'}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <button
                onClick={() => patchClimate({ tempRear: Math.max(16, climate.tempRear - 0.5) })}
                disabled={climate.tempSync}
                style={{ background: 'none', border: 'none', cursor: climate.tempSync ? 'not-allowed' : 'pointer', fontSize: 16, color: 'var(--text-3)', padding: '0 2px', lineHeight: 1, opacity: climate.tempSync ? 0.3 : 1 }}
              >−</button>
              <span style={{ fontSize: 26, fontWeight: 900, letterSpacing: '-0.04em', lineHeight: 1 }}>
                {climate.tempRear.toFixed(1)}
                <span style={{ fontSize: 11, fontWeight: 400, color: 'var(--text-3)', marginLeft: 1 }}>°C</span>
              </span>
              <button
                onClick={() => patchClimate({ tempRear: Math.min(30, climate.tempRear + 0.5) })}
                disabled={climate.tempSync}
                style={{ background: 'none', border: 'none', cursor: climate.tempSync ? 'not-allowed' : 'pointer', fontSize: 16, color: 'var(--text-3)', padding: '0 2px', lineHeight: 1, opacity: climate.tempSync ? 0.3 : 1 }}
              >+</button>
            </div>
            ...
            <div style={{ display: 'flex', gap: 3 }}>
              {AIRFLOW_MODES.map(({ mode, icon, title }) => (
                <button
                  key={mode}
                  title={title}
                  onClick={() => patchClimate({ airflowModeRear: mode })}
                  style={{
                    width: 22, height: 22,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    background: climate.airflowModeRear === mode ? 'var(--text)' : 'transparent',
                    border: `1px solid ${climate.airflowModeRear === mode ? 'var(--text)' : 'var(--border-light)'}`,
                    color: climate.airflowModeRear === mode ? 'var(--bg)' : 'var(--text-3)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.7fr', gridTemplateRows: 'auto 1fr', border: '1px solid var(--border-light)', margin: '0 -12px -8px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--surface)', padding: '4px 8px', borderBottom: '1px solid var(--border-light)' }}>
          ...
          <button
            className={`swiss-chip${seatPassenger.zeroGravity ? ' accent' : ''}`}
            style={{ cursor: 'pointer', fontSize: 9 }}
            onClick={toggleZeroGravity}
          >
            {seatPassenger.zeroGravity ? '零重力 ON' : '零重力'}
          </button>
        </div>
        ...
        <div style={{ borderRight: '1px solid var(--border-light)', padding: '6px 8px 10px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ flex: 1, position: 'relative', overflow: 'hidden', minHeight: 52 }}>
            <div style={{
              ...
              height: `${100 - windows.passengerWindow}%`,
            ...
            <div style={{
              ...
              <button
                onClick={() => patchWindows({ passengerWindow: Math.max(0, windows.passengerWindow - 50) })}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', padding: 2, lineHeight: 1 }}
              >
                <Icon name="winUp" size={16} />
              </button>
              <button
                onClick={() => patchWindows({ passengerWindow: Math.min(100, windows.passengerWindow + 50) })}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', padding: 2, lineHeight: 1 }}
              >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, marginBottom: 8 }}>
            <div>
              ...
              <div style={{ fontSize: 11, fontWeight: 700 }}>
                {seatPassenger.foreAft < 50 ? '偏前' : seatPassenger.foreAft > 50 ? '偏后' : '居中'}
              </div>
            </div>
            <div>
              <div className="swiss-label" style={{ marginBottom: 2 }}>靠背角度</div>
              <div style={{ fontSize: 11, fontWeight: 700 }}>{seatPassenger.recline}°</div>
            </div>
          </div>

          <div style={{ display: 'flex', gap: 4 }}>
            {([
              { icon: 'ventilation' as const, val: seatPassenger.ventilation !== 'off', action: () => patchSeatPassenger({ ventilation: seatPassenger.ventilation === 'off' ? 'low' : 'off' }), title: '通风' },
              { icon: 'heating'     as const, val: seatPassenger.heating,               action: () => patchSeatPassenger({ heating: !seatPassenger.heating }),                                    title: '加热' },
              { icon: 'massage'     as const, val: seatPassenger.massage,               action: () => patchSeatPassenger({ massage: !seatPassenger.massage }),                                    title: '按摩' },
